...
# ----------------------- Fix the GenerationConfig.from_model_config bug for Qwen2.5-VL -----------------------
from transformers.generation.configuration_utils import GenerationConfig
from transformers.configuration_utils import PretrainedConfig

class DictConfigWrapper:
    """Wrapper to make a dict look like a PretrainedConfig with to_dict() method."""
    def __init__(self, d):
        self._dict = d
        # Copy all dict items as attributes for compatibility
    ...
    def __getattr__(self, name):
        """Fallback to dict lookup if attribute not found."""
        if '_dict' in self.__dict__ and name in self._dict:
            return self._dict[name]
        raise AttributeError(f"'{type(self).__name__}' object has no attribute '{name}'")

def monkey_patch_generation_config():
    """
    Monkey patch GenerationConfig.from_model_config to handle the case where
    decoder_config/text_config is already a dict instead of a PretrainedConfig object.
    
    This fixes the AttributeError: 'dict' object has no attribute 'to_dict'
    that occurs when loading Qwen2.5-VL models.
    """
    # Store original method if not already stored
    if not hasattr(GenerationConfig, '_original_from_model_config'):
        GenerationConfig._original_from_model_config = GenerationConfig.from_model_config
    
    def patched_from_model_config(cls, config, **kwargs):
        """
        Patched version that handles dict sub-configs (decoder_config, text_config, decoder).
        """
        # List of attributes that might be dict and need wrapping
        # These are the attributes that transformers' from_model_config might try to call to_dict() on
        config_attrs_to_check = ['decoder_config', 'text_config', 'decoder']
        
        # Store original values and wrap dicts
        original_values = {}
        for attr in config_attrs_to_check:
            if hasattr(config, attr):
                value = getattr(config, attr)
                if isinstance(value, dict):
                    original_values[attr] = value
                    setattr(config, attr, DictConfigWrapper(value))
        
        try:
            # Call original method
            return GenerationConfig._original_from_model_config.__func__(cls, config, **kwargs)
        finally:
